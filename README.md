...
The **Diagnostics** tab ends with a chart and table showing what happens if the cell is cycled away from the rate it was designed for. It opens with one sentence — *sized at C/10 (10 h) to deliver 315 µAh at N/P 1.00* — and then answers a single question for each rate: how much capacity survives, and is the cell still balanced.

**The chart** draws two lines, cathode and anode, in the same green and blue the capacity bars above use for the same two electrodes. **The cell delivers whichever line is lower**, so the shaded area beneath them is the deliverable capacity. A dot marks a real measurement and the line runs solid through measured ground and dashed where the value was estimated; a slim green/amber/red strip along the bottom shows where the balance still holds, and a dashed marker says *designed here*.

**The table** has three columns — the rate, the capacity kept (as a percentage, with a bar you can scan down), and the balance stated in words (*on target*, *drifting · anode-limited*, *off design · anode-limited*). Clicking a row opens everything else for that rate: Q<sub>c</sub> and Q<sub>a</sub>, the exact N/P and its deviation from target, both electrode currents, the potential shift of the part-swept electrode, where each capacity came from, and a button to simulate the cell at that rate. The **Rate** header doubles as a switch between cell C-rate, cathode mA/g and anode A/g.

Capacity at each rate is **extrapolated from the material's measured half-cell points**: the library's (rate, capacity) pairs are fitted against log(current), interpolated between points and extended up to a decade beyond them. The table reports a fixed ladder — C/20, C/10, C/5, C/2, 1C, 2C, 5C — plus your design rate. A checkbox adds the measured points themselves as extra rows.

Where the data cannot support an estimate — a single measured rate, no rate data at all, or a rate far beyond the ladder — the tool **says so instead of inventing a number**, once, rather than repeating itself down the table. If nothing at all can be estimated the section is replaced by a single card explaining why. A second checkbox, *assume capacity is constant outside the measured range*, fills those regions in: by default each end holds its own nearest measured value, and you can type a capacity per electrode to override it.
